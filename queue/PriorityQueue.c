...
void decrease_priority(heaptype heap, int taskid, int priotity) {
	int where = gettaskbyid(heap, taskid);
	if (where == -1 || where >= heap->size || heap->data[where]->priority <= priotity)
		return;
	heap->data[where]->priority = priotity;
	hold_heap(heap, where);
}
void increase_priority(heaptype heap, int taskid, int priotity) {
	int where = gettaskbyid(heap, taskid);
	if (where == -1 || where >= heap->size || heap->data[where]->priority >= priotity)
		return;
	int currentpos = where;
	int parentpos = 0;
	heap->data[where]->priority = priotity;
	while ((parentpos = (currentpos - 1) >> 1) >= 0 &&
		heap->data[parentpos]->priority < heap->data[currentpos]->priority) {
